...
class ExceptionListenerWrapper
{
    ...
    public function __invoke(GetResponseForExceptionEvent $event)
    {
        ...
        $this->callback = $this->container['callback_resolver']->resolveCallback($this->callback);
    ...
    protected function ensureResponse($response, GetResponseForExceptionEvent $event)
    {
        if ($response instanceof Response) {
            ...
            $viewEvent = new GetResponseForControllerResultEvent($this->container['kernel'], $event->getRequest(), $event->getRequestType(), $response);
            $this->container['event_dispatcher']->dispatch(KernelEvents::VIEW, $viewEvent);
